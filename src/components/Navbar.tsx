...
export function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 backdrop-blur-2xl bg-white/80 border-b border-gray-200">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-500 to-orange-500 flex items-center justify-center shadow-lg shadow-blue-500/15">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <Link href="/blog" className="text-sm text-gray-500 hover:text-gray-900 transition-colors">Resources</Link>
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="md:hidden border-t border-gray-200 bg-white/95 backdrop-blur-2xl">
            <div className="px-6 py-4 flex flex-col gap-4">
              <Link href="/providers" onClick={() => setOpen(false)} className="text-gray-600">Browse Providers</Link>
              <Link href="/compare" onClick={() => setOpen(false)} className="text-gray-600">Compare</Link>
              <Link href="/pricing" onClick={() => setOpen(false)} className="text-gray-600">Pricing</Link>
              <Link href="/blog" onClick={() => setOpen(false)} className="text-gray-600">Resources</Link>
