...
function NotificationBell({ userEmail }: { userEmail: string }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={handleOpen}
        className="relative p-2 rounded-lg text-ink-500 hover:text-ink-700 hover:bg-gray-100 transition-colors"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -6 }}
            className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-lg border border-line-100 overflow-hidden z-50"
          >
            ...
            {notifications.length === 0 ? (
              <div className="px-4 py-6 text-center text-sm text-ink-400">
                <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24" className="mx-auto mb-2 text-ink-300">
                  <path d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                </svg>
                    ...
                    <div key={n.id} className={`px-4 py-3 ${isRead ? "opacity-60" : ""}`}>
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-ink-500 mt-0.5">{n.service || "General enquiry"}</p>
                          <p className="text-[10px] text-ink-400 mt-0.5">{n.created_at ? new Date(n.created_at).toLocaleDateString() : ""}</p>
                        </div>
                      </div>
                    </div>
                  );
            ...
            <div className="px-4 py-2.5 border-t border-line-100">
    ...
    <div className="flex items-center gap-1">
      ...
      <div className="relative" ref={ref}>
        <button
          ...
          className="flex items-center gap-2 px-3 py-2 rounded-lg border border-line-200 hover:border-gray-300 text-sm text-ink-700 hover:text-ink-900 transition-all"
        >
        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -6 }}
              className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-line-100 py-1 z-50"
            >
              <Link
                href="/dashboard"
              ...
              <hr className="my-1 border-line-100" />
              <button
        ...
        {open && (
          <>
            ...
            >
              ...
              <nav className="flex-1 px-5 py-4 flex flex-col gap-1 overflow-y-auto border-t border-line-100">
                {NAV_LINKS.map(({ label, href }) => {
                  const active = pathname === href || pathname.startsWith(href + "/");
                  return (
                    <Link
                ...
                {user && (
                  <Link href="/dashboard" className="flex items-center px-3 py-2.5 rounded-lg text-sm font-medium text-ink-700 hover:bg-gray-50">
                    Dashboard
                  </Link>
                )}
              </nav>
              ...
              {user && (
                <div className="px-5 pb-8 pt-4 border-t border-line-100 flex flex-col gap-3">
                  <p className="text-xs text-ink-500 truncate px-1">{user.email}</p>
                  <button
