    ...
    <AnimatePresence>
      ...
      >
        ...
        <motion.div
          ...
          className="relative w-full max-w-lg rounded-2xl bg-white border border-line-200 shadow-2xl p-6 sm:p-8 max-h-[90vh] overflow-y-auto"
        >
          ...
          {sent ? (
            <div className="text-center py-8">
              ...
              <h3 className="text-xl font-bold text-ink-900 mb-2">Enquiry sent!</h3>
              <p className="text-sm text-ink-500">{providerName} will receive your message and respond shortly.</p>
            </div>
          ) : (
            <>
              <h2 className="text-xl font-bold text-ink-900 mb-1">Send Enquiry</h2>
              <p className="text-sm text-ink-500 mb-6">to {providerName}</p>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-medium text-ink-700 mb-1.5 block">Your Name *</label>
                    <input type="text" required value={form.name} onChange={e => updateForm({ name: e.target.value })}
                      className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-line-200 text-ink-900 placeholder-gray-400 text-sm focus:outline-none focus:border-orange-500 transition-colors" />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-ink-700 mb-1.5 block">Phone</label>
                ...
                <div>
                  <label className="text-xs font-medium text-ink-700 mb-1.5 block">Service Needed</label>
                  <select value={form.service} onChange={e => updateForm({ service: e.target.value })}
                    className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-line-200 text-ink-700 text-sm focus:outline-none focus:border-orange-500 transition-colors appearance-none">
                ...
                <div>
                  ...
                  <textarea required rows={4} value={form.message} onChange={e => updateForm({ message: e.target.value })}
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-line-200 text-ink-900 placeholder-gray-400 text-sm focus:outline-none focus:border-orange-500 transition-colors resize-none" />
                </div>
                ...
                <p className="text-xs text-ink-400 text-center">Your details will only be shared with this provider.</p>
              </form>
            </>
          )}
        </motion.div>
