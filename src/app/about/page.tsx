      ...
      <section className="px-6 py-20 bg-surface/50">
        <div className="max-w-4xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <div className="rounded-2xl bg-surface border border-white/[0.06] p-10 sm:p-14">
              <span className="text-xs font-semibold tracking-widest uppercase text-blue-400 mb-4 block">Our Mission</span>
              <p className="text-2xl sm:text-3xl font-bold leading-snug text-white/90">
          ...
          <div className="space-y-8">
            {timeline.map((t, i) => (
              <motion.div key={t.year} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                ...
                <div className="border-l border-white/[0.08] pl-6 pb-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {team.map((t, i) => (
              <motion.div key={t.name} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                className="rounded-2xl bg-surface border border-white/[0.06] p-8">
                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-blue-600/20 to-orange-500/20 border border-white/[0.08] flex items-center justify-center mb-4">
      ...
      <section className="px-6 py-20 bg-surface/50">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Link href="/register" className="px-8 py-3.5 rounded-xl bg-white/[0.06] hover:bg-white/[0.1] text-white font-medium border border-white/[0.08] transition-all">
              List Your Service
            </Link>
          </div>
        </div>
