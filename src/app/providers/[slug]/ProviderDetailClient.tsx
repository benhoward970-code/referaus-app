  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const dbRow = await getProviderBySlug(slug);
        if (!cancelled && dbRow) {
          setProvider(mapDbProvider(dbRow));
        }

        // Fetch real reviews
        const dbReviews = await getProviderReviews(slug);
        if (!cancelled && dbReviews.length > 0) {
          setReviews(
        ...
        console.error("[provider-detail] DB fetch failed, using fallback:", err);
      } finally {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col md:flex-row gap-8 mb-12">
          {provider.logo_url ? (
            ...
            <div
              className="w-24 h-24 rounded-2xl border border-gray-200 flex items-center justify-center shrink-0"
              style={{ background: `linear-gradient(135deg, ${accentLight}, rgba(249,115,22,0.12))` }}
            >
              <span className="text-4xl font-bold" style={{ color: accent }}>{provider.name[0]}</span>
            </div>
          )}
