    ...
    <div className="min-h-screen pt-24 pb-20 px-6">
      <div className="max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <p className="text-gray-500 text-sm">Welcome back, Sunshine Support Services</p>
        </motion.div>

        {/* Stats */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {stats.map((s, i) => (
            <motion.div key={s.label} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="rounded-2xl bg-surface border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Enquiries table */}
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="lg:col-span-2 rounded-2xl bg-surface border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              {recentEnquiries.map((e) => (
                <div key={e.name} className="flex items-center justify-between py-3 border-b border-gray-200 last:border-0">
                  <div>
          ...
          {/* Quick actions */}
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="rounded-2xl bg-surface border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              ...
              <button className="w-full py-3 rounded-xl bg-gray-50 hover:bg-gray-50 text-gray-500 text-sm font-medium border border-gray-200 transition-all">Manage Services</button>
              <button className="w-full py-3 rounded-xl bg-gray-50 hover:bg-gray-50 text-gray-500 text-sm font-medium border border-gray-200 transition-all">View Analytics</button>
              <button className="w-full py-3 rounded-xl bg-gray-50 hover:bg-gray-50 text-gray-500 text-sm font-medium border border-gray-200 transition-all">Respond to Reviews</button>
            </div>
            <div className="mt-6 p-4 rounded-xl bg-orange-500/[0.06] border border-orange-500/20">
              ...
              <p className="text-xs text-gray-500">Get verified, priority ranking, and direct bookings.</p>
