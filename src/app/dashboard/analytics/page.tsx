      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-ink-500 text-sm mt-1">How participants are finding and engaging with your listing</p>
          </div>
          <div className="flex gap-1 bg-gray-100 rounded-lg p-1">
            {(['7d', '30d', '90d'] as const).map(p => (
              <button key={p} onClick={() => setPeriod(p)} className={'px-3 py-1.5 rounded-md text-xs font-medium transition-all ' + (period === p ? 'bg-white text-ink-900 shadow-sm' : 'text-ink-500 hover:text-ink-700')}>{p}</button>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            <motion.div key={s.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="bg-white border border-line-200 rounded-xl p-5">
        ...
        <div className="bg-white border border-line-200 rounded-xl p-6 mb-6">
          <h2 className="font-bold mb-1">Enquiries — Last 7 Days</h2>
          <p className="text-xs text-ink-400 mb-6">Number of enquiries received per day</p>
          {totalEnquiries === 0 ? (
              ...
              {weeklyData.map((d, i) => {
                ...
                return (
                  <div key={d.day} className="flex-1 flex flex-col items-center gap-2">
                    <span className="text-xs text-ink-500 font-medium">{d.enquiries > 0 ? d.enquiries : ""}</span>
        ...
        <div className="bg-white border border-line-200 rounded-xl p-6 mb-8">
          <h2 className="font-bold mb-1">Profile Views — This Week</h2>
          <p className="text-xs text-ink-400 mb-6">Number of times your profile was viewed</p>
          {totalViews === 0 ? (
            <div className="flex flex-col items-center justify-center h-40 text-ink-400">
              <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="mb-3 opacity-40">
            ...
            <div className="flex items-end gap-3 h-40">
              {weeklyData.map((d, i) => (
                <div key={d.day} className="flex-1 flex flex-col items-center gap-2">
                  <span className="text-xs text-ink-500 font-medium">{d.views}</span>
                  <div className="w-full flex-1 flex items-end">
                    <motion.div
                      initial={{ height: 0 }}
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-white border border-line-200 rounded-xl p-6">
            ...
            <div className="space-y-4">
              {topServices.map(s => (
                <div key={s.name}>
                  <div className="flex justify-between text-sm mb-1"><span className="text-ink-700">{s.name}</span><span className="text-ink-400">{s.views} views</span></div>
            ...
            <div className="space-y-3">
              {topSearchTerms.map((t, i) => (
                <div key={t.term} className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-ink-700 flex-1">{t.term}</span>
        ...
        <p className="text-center text-xs text-ink-400 mt-8">Data shown is demo data. Connect Supabase to see real analytics.</p>
      </motion.div>
    </div>
  );
}
