  ...
  return (
    <div className="min-h-screen pt-24 pb-20 px-4 sm:px-6 max-w-xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-ink-500 text-sm mb-8">Help other participants find the right providers by sharing your experience</p>

        <div className="space-y-5">
          <div><label className="block text-sm font-medium mb-1.5">Your Name</label><input value={form.name} onChange={e => update('name', e.target.value)} className="w-full px-4 py-3 rounded-lg border border-line-200 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" placeholder="First name or initials" /></div>
          <div><label className="block text-sm font-medium mb-1.5">I am a...</label>
            <div className="flex gap-3">
              {['participant', 'family member', 'support coordinator'].map(r => (
                <button key={r} onClick={() => update('role', r)} className={'px-4 py-2.5 rounded-lg text-sm font-medium border transition-all ' + (form.role === r ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-ink-700 border-line-200 hover:border-blue-300')}>{r.charAt(0).toUpperCase() + r.slice(1)}</button>
              ))}
            </div>
          </div>
          <div><label className="block text-sm font-medium mb-1.5">Provider Name (optional)</label><input value={form.provider} onChange={e => update('provider', e.target.value)} className="w-full px-4 py-3 rounded-lg border border-line-200 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" placeholder="Which provider did you use?" /></div>
          ...
          <div><label className="block text-sm font-medium mb-1.5">Your Experience</label><textarea value={form.message} onChange={e => update('message', e.target.value)} rows={5} className="w-full px-4 py-3 rounded-lg border border-line-200 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 resize-none" placeholder="How has the NDIS or your provider helped you? What would you tell other participants?" /></div>
          ...
          <p className="text-xs text-ink-400 text-center">Testimonials are reviewed before being published. We may edit for clarity.</p>
        </div>
      </motion.div>
    </div>
  );
